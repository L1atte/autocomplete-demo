...
import EpsilonTransition from "antlr4/src/antlr4/transition/EpsilonTransition";
import RuleTransition from "antlr4/src/antlr4/transition/RuleTransition";
import AtomTransition from "antlr4/src/antlr4/transition/AtomTransition";
import SetTransition from "antlr4/src/antlr4/transition/SetTransition";
import ActionTransition from "antlr4/src/antlr4/transition/ActionTransition";
...
function App() {
	...
	function handleEditorDidMount(_, editor) {
		...
		editor.onKeyUp(ev => {
			...
			const currentMouseData = model.getLineContent(position.lineNumber).slice(0, position.column - 1);
			...
			// });
			if (ev.keyCode === 68) {
				console.log("currentMouseData:", currentMouseData);
				const queryPredict = predict(fields, functions, currentMouseData);
				console.log(queryPredict);
			}
		});
	...
	const describeTransition = t => {
		...
		if (t instanceof PrecedencePredicateTransition) return "precedence predicate " + t.precedence;

		return "UNKNOWN" + t.constructor.name;
	};

	const describeState = s => {
		if (s instanceof RuleStartState) return `rule start (stop -> ${s.stopState}) isLeftRec ${s.isLeftRecursiveRule} (ruleIndex=${s.ruleIndex})`;
		if (s instanceof RuleStopState) return `rule stop (ruleIndex=${s.ruleIndex})`;
		if (s instanceof BasicState) return `basic`;
		if (s instanceof PlusBlockStartState) return `plus block start (loopback ${s.loopBackState})`;
		if (s instanceof StarBlockStartState) return `star block start`;
		if (s instanceof StarLoopEntryState) return `star loop entry start (loopback ${s.loopBackState}) prec ${s.isPrecedenceDecision}`;
		...
		if (s instanceof LoopEndState) return `loop end (loopback ${s.loopBackState})`;
	...
	const autoCompleteProcess = (state, tokens, collector, parserStack, alreadyPassed, history) => {
		...
		for (let transition of state.transitions) {
			...
			if (transition.isEpsilon) {
				...
				if (atCaret) {
					if (isCompatibleWithStack(target, parserStack)) collector.collect(label);
				} else {
					if (nextTokenType.type === label) autoCompleteProcess(target, tokens.move(), collector, stackRes.states, [], history);
				}
			} else if (transition instanceof SetTransition) {
				const nextTokenType = tokens.next();
				for (let symbol of getLabel(transition.label.intervals)) {
					if (atCaret) {
						if (isCompatibleWithStack(target, parserStack)) collector.collect(symbol);
					} else {
						if (nextTokenType.type === symbol) autoCompleteProcess(target, tokens.move(), collector, stackRes.states, [], history);
					}
				}
			} else {
